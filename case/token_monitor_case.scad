...
// сзади. Это единственный размер, который задаёт толщину корпуса.
//   terminal_cut = false — корпус закрывает клеммник целиком,
//                          stack_h = 15 (до колодки) + высота колодки ≈ 10.
//   terminal_cut = true  — в крышке проём под колодку, корпус тоньше,
//                          тогда stack_h = 15.
terminal_cut = false;
stack_h      = terminal_cut ? 15.0 : 25.0;

/* [Проём под клеммник, если terminal_cut = true] */
term_w       = 46;     // ширина проёма (колодка 41 мм + запас)
term_h       = 15;     // высота проёма
term_x       = 0.6;    // центр колодки по горизонтали
term_y       = -31;    // центр колодки по вертикали
...
module back_shell() {
    difference() {
        ...
        // проём под клеммник — только в тонком варианте
        if (terminal_cut)
            translate([term_x, term_y, -0.1])
                rrect(term_w, term_h, 2, back_t + 0.2);
    }
}
