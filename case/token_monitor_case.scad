...
/* [Глубина — сверить с реальной платой!] */
module_gap   = 3.0;    // от задней поверхности стекла до лица платы
back_clear   = 11.4;   // место за платой под клеммник и модуль ESP32
                       // (клеммник ~10.5 мм — самая высокая деталь)
...
depth_in    = module_gap + pcb_t + back_clear; // от стекла до задней крышки
